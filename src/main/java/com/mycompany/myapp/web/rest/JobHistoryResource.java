...
public class JobHistoryResource {
    ...
    private static final String ENTITY_NAME = "testJhipsterMongodbJobHistory";
    ...
    public ResponseEntity<JobHistory> createJobHistory(@RequestBody JobHistory jobHistory) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/job-histories/" + jobHistory.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, jobHistory.getId()))
            .body(jobHistory);
    }
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<JobHistory> getJobHistory(@PathVariable("id") String id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteJobHistory(@PathVariable("id") String id) {
